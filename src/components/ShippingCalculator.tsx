...
const ShippingCalculator = () => {
  ...
  const availableMethods = selectedCountry === "Paraguay"
  ...
  const animatedTotal = useAnimatedNumber(shippingResults?.shippingCost || 0, 1000);
    ...
    <Card className="rounded-3xl p-8 bg-white/90 backdrop-blur-sm shadow-lg border-2 border-white/30">
      <CardHeader className="pb-4">
        <CardTitle className="flex items-center gap-3 font-heading text-xl text-gray-800 leading-tight flex-wrap">
          ...
          <span className="whitespace-normal break-words">{t('shippingCalculator')}</span>
        </CardTitle>
      ...
      <CardContent className="space-y-4">
        <div className="space-y-1">
          ...
          {!zoneInfo && !isDHLOnly && (
            <p className="text-xs text-gray-600 font-body text-center">Select a country</p>
          )}
        </div>

        <div className="space-y-2">
          <Label className="font-body text-sm text-gray-700">{t('shippingMethods')}</Label>
          ...
          >
            {availableMethods.map((method) => (
              <div key={method.value} className="flex items-center space-x-2 p-2 rounded-full bg-capybara-cream transition-colors">
                <RadioGroupItem value={method.value} id={method.value} className="w-3 h-3" />
                <Label htmlFor={method.value} className="flex-1 font-body cursor-pointer text-xs">
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-3 gap-2">
                <div>
                  <Label htmlFor="length" className="text-xs">{t('lengthLabel')}</Label>
                  <Input
                    id="length"
                    type="number"
                    min="1"
                ...
                <div>
                  <Label htmlFor="width" className="text-xs">{t('widthLabel')}</Label>
                  <Input
                    id="width"
                    type="number"
                    min="1"
                ...
                <div>
                  <Label htmlFor="height" className="text-xs">{t('heightLabel')}</Label>
                  <Input
                    id="height"
                    type="number"
                    min="1"
        ...
        <div className="space-y-2">
          <Label className="font-body text-sm text-gray-700">{t('selectWeight')}</Label>
          <div className="flex items-center gap-4">
            <Input
              ...
              className="w-32"
            />
            <span className="text-sm text-muted-foreground whitespace-nowrap">g</span>
            <Slider
              value={weight}
        ...
        {shippingResults && (
          <div className="space-y-3 mt-4 animate-fade-in animate-scale-in">
            <div className="bg-gradient-to-br from-gray-800 to-gray-900 text-white rounded-2xl p-4 transition-all duration-500">
              <div className="text-2xl font-bold text-center">{animatedTotal.toLocaleString()} yen</div>
              <p className="text-gray-300 text-sm text-center">{t('internationalShippingCost')}</p>
            </div>
            <div className="space-y-2">
              <div className="border-t-2 border-capybara-orange pt-2 space-y-2">
                <div className="flex justify-between items-center py-2 px-3 bg-capybara-yellow rounded-full text-sm">
                  ...
                  <span className="font-heading font-bold text-gray-800">${shippingResults.usd}</span>
                </div>
                <div className="flex justify-between items-center py-2 px-3 bg-capybara-yellow rounded-full text-sm hidden">
                  ...
                  <span className="font-heading font-bold text-gray-800">{shippingResults.clp}</span>
                </div>
                <div className="flex justify-between items-center py-2 px-3 bg-capybara-yellow rounded-full text-sm hidden" >
                  <span className="font-body text-gray-700">MXN</span>
                  <span className="font-heading font-bold text-gray-800">{shippingResults.mxn}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        {!shippingResults && (
          <div className="text-center py-8">
            <div className="mb-4">
              <img src="KapyPlane.png" alt="Capybara Plane" className="w-28 h-36 mx-auto mb-2" />
            </div>
            <p className="font-body text-gray-600 text-sm">{t('enterDataToCalculate')}</p>
          </div>
        )}
      </CardContent>
    </Card>
