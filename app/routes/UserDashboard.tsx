...
import { useSearchParams, useNavigate, NavigateFunction } from 'react-router';
...
const UserDashboardContent = () => {
  ...
  const validTabs = ['submit', 'orders', 'storage', 'shipping', 'profile', 'notifications', 'help'];
  const activeTab = tabParam && validTabs.includes(tabParam) ? tabParam : 'submit';
  ...
  const helpItem = { id: 'help', label: 'Ayuda', desc: 'Centro de ayuda', icon: HelpCircle };

  const submitItem = { id: 'submit', label: t('submitRequest'), desc: t('submitRequestDesc'), icon: Plus };

  const navigationItems = [submitItem, ...bottomNavItems, helpItem];
