...
export const ShippingRequestsManagement = React.memo(({ shipmentId }: ShippingRequestsManagementProps) => {
  const { toast } = useToast();
  const queryClient = useQueryClient();
  const [openSections, setOpenSections] = useState<Record<string, boolean>>({});
  const [searchTerm, setSearchTerm] = useState('');
  const [rejectDialogOpen, setRejectDialogOpen] = useState(false);
  const [rejectingRequest, setRejectingRequest] = useState<ShippingRequest | null>(null);
  const [rejectionReason, setRejectionReason] = useState('');
  const [productIssues, setProductIssues] = useState<ProductIssue[]>([]);
  const [trackingUrl, setTrackingUrl] = useState('');
  const [trackingUrls, setTrackingUrls] = useState<{[itemId: string]: string}>({});
  const [currentShipmentBoxItems, setCurrentShipmentBoxItems] = useState<ProductRequest[]>([]);
  ...
  const [sendQuoteRequestId, setSendQuoteRequestId] = useState<string | null>(null);
  ...
  const [editTrackingRequestId, setEditTrackingRequestId] = useState<string | null>(null);
  ...
  const [editBoxTrackingRequestId, setEditBoxTrackingRequestId] = useState<string | null>(null);
  const [editBoxTrackingItemId, setEditBoxTrackingItemId] = useState<string | null>(null);
  ...
  const [paymentRequestForConfirmation, setPaymentRequestForConfirmation] = useState<ShippingRequest | null>(null);
  const [isConfirmingPayment, setIsConfirmingPayment] = useState(false);
  
  // Mark as Sent Confirmation Dialog
  const [markAsSentDialogOpen, setMarkAsSentDialogOpen] = useState(false);
  const [requestForMarkAsSent, setRequestForMarkAsSent] = useState<ShippingRequest | null>(null);
    ...
    queryFn: async () => {
      ...
      const processedQuotes = await Promise.all(quotes.map(async (quote) => {
        ...
        const itemsWithDetails = await Promise.all(items.map(async (item: any) => {
          if (item.order_item_id) {
            ...
            const { data: productRequest } = await supabase
              ...
              .select('id, product_url, item_name, quantity, weight, has_shipping_issue, shipping_issue_description, is_box, local_tracking_number')
        ...
        return {
          ...
          tracking_urls: (quote.tracking_urls as unknown as TrackingUrlEntry[] | null) || undefined,
  ...
  const sendQuoteMutation = useMutation({
    ...
    onSuccess: async (_, variables) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['admin-shipping-requests'] });
      toast({
        title: 'Quote sent',
  ...
  const updateStatusMutation = useMutation({
    mutationFn: async ({
      ...
      const { data: request } = await supabase
        .from('shipping_quotes')
        .select('user_id, destination, tracking_url, tracking_urls, shipment_personal_id')
        .eq('id', requestId)
        .single();
      ...
      if (request) {
        if (status === 'paid') {
          ...
          let trackingMsg = '';
          const trackingUrlsArray = request.tracking_urls as unknown as TrackingUrlEntry[] | null;
  ...
  const updateBoxTrackingUrlMutation = useMutation({
    mutationFn: async ({
      ...
      const currentUrls = (request.tracking_urls as unknown as TrackingUrlEntry[]) || [];
  ...
  const handleRefresh = async () => {
    setRefreshing(true);
    try {
      await queryClient.invalidateQueries({ queryKey: ['admin-shipping-requests'] });
      toast({
        title: 'Updated',
  ...
  const updateProductIssue = (productId: string, field: 'hasIssue' | 'issueDescription', value: boolean | string) => {
    setProductIssues(prev => prev.map(issue => {
      if (issue.productId === productId) {
        if (field === 'hasIssue') {
          return { ...issue, hasIssue: value as boolean, issueDescription: value ? issue.issueDescription : '' };
        } else {
  ...
  const handleConfirmSent = () => {
    ...
    updateStatusMutation.mutate({
      ...
      tracking_urls: trackingUrlsArray.length > 0 ? trackingUrlsArray : undefined
    });
  };
  ...
  const handleEditBoxTrackingUrl = (requestId: string, itemId: string, currentUrl: string) => {
  ...
  const getStatusSteps = (currentStatus: string) => {
    const baseSteps = [
      { label: 'Request Received', status: 'completed' as const, icon: <Package className="h-4 w-4" /> },
      { label: 'Quote Sent', status: 'upcoming' as const, icon: <Send className="h-4 w-4" /> },
      { label: 'Payment Confirmed', status: 'upcoming' as const, icon: <CreditCard className="h-4 w-4" /> },
      { label: 'Shipment Sent', status: 'upcoming' as const, icon: <Truck className="h-4 w-4" /> },
    ];

    if (currentStatus === 'rejected') {
      return [
        { label: 'Request Received', status: 'completed' as const, icon: <Package className="h-4 w-4" /> },
        { label: 'Rejected', status: 'rejected' as const, icon: <AlertCircle className="h-4 w-4" /> },
      ];
    }
    ...
    if (currentStatus === 'cancelled') {
      return [
        { label: 'Request Received', status: 'completed' as const, icon: <Package className="h-4 w-4" /> },
        { label: 'Cancelled', status: 'rejected' as const, icon: <X className="h-4 w-4" /> },
      ];
    }
    ...
    return baseSteps.map((step, index) => ({
      ...step,
      status: (index <= statusIndex ? 'completed' : 'upcoming') as 'completed' | 'upcoming'
    }));
  };
    ...
    return (
      <Card>
        <CardContent className="p-8">
          <div className="text-center text-muted-foreground">Loading shipping requests...</div>
        </CardContent>
      </Card>
    );
  }
    ...
    <>
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Manage all shipping quote requests with complete shipping information
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="mb-6">
            ...
            <CardContent className="p-4">
              ...
              {filteredRequests.length > 0 && (
                <div className="mt-4 text-sm text-muted-foreground">
                  Showing {startIndex + 1}-{Math.min(endIndex, filteredRequests.length)} of {filteredRequests.length} shipping requests
                </div>
              )}
            </CardContent>
          </Card>
          ...
          <div className="space-y-4">
            {filteredRequests.length === 0 && shippingRequests && shippingRequests.length > 0 && (
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                        ...
                        <div className="flex flex-col text-left flex-1 space-y-1">
                          <p className="font-medium text-base">
                            <span className="select-text" onMouseDown={(e) => e.stopPropagation()} onClick={(e) => e.stopPropagation()}>Shipment for </span>
                            <span className="text-blue-500 font-semibold select-text" onMouseDown={(e) => e.stopPropagation()} onClick={(e) => e.stopPropagation()}>
                              {request.user.full_name || request.user.email || "Unknown"}
                            </span>
                          </p>

                          <p className="text-sm text-muted-foreground">
                            <span className="select-text" onMouseDown={(e) => e.stopPropagation()} onClick={(e) => e.stopPropagation()}>Customer ID: </span>
                            <span className="font-medium text-emerald-500 select-text" onMouseDown={(e) => e.stopPropagation()} onClick={(e) => e.stopPropagation()}>
                              {request.user.user_personal_id ? `#${request.user.user_personal_id}` : "N/A"}
                            </span>
                          </p>

                          <p className="text-sm text-muted-foreground">
                            <span className="select-text" onMouseDown={(e) => e.stopPropagation()} onClick={(e) => e.stopPropagation()}>Customer Shipment ID: </span>
                            <span className="font-medium text-emerald-500 select-text" onMouseDown={(e) => e.stopPropagation()} onClick={(e) => e.stopPropagation()}>
                              {request.shipment_personal_id ? `#${request.shipment_personal_id}` : "N/A"}
                            </span>
                          </p>

                          <div className="flex items-center gap-1 text-sm text-muted-foreground">
                            <span className="font-medium select-text" onMouseDown={(e) => e.stopPropagation()} onClick={(e) => e.stopPropagation()}>
                              Shipment #{request.id.substring(0, 8).toUpperCase()}
                            </span>
                            <Button
                              variant="ghost"
                              size="icon"
                              ...
                              onClick={(e) => {
                                ...
                                toast({ title: "Shipment ID copied to clipboard" });
                              }}
                  ...
                  <Card className="mt-2">
                    <CardContent className="pt-4">
                      ...
                      <div className="mb-6">
                        ...
                        <StatusFlow steps={getStatusSteps(request.status)} />
                      </div>
                            ...
                            <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                              ...
                              <div className="space-y-3">
                                {boxItems.map((item, index) => {
                                  const trackingEntry = request.tracking_urls?.find(entry => entry.item_id === item.id);
                                      ...
                                      <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                          <p className="text-sm font-medium text-blue-900">
                                            Box {index + 1}: {item.item_name|| 'No tracking'}
                                          </p>
                                          {item.item_name && (
                                            <p className="text-xs text-blue-700">{item.local_tracking_number}</p>
                                          )}
                                        </div>
                                      </div>
                                      <div className="flex items-center gap-2">
                           ...
                           return (
                            <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                              <div className="flex items-center justify-between">
                                <div>
                                  <h4 className="font-medium text-blue-800 mb-1">Tracking Information</h4>
                                  <a
                      ...
                      <div className="mb-4 p-3 bg-accent/20 rounded-lg">
                        ...
                        <div className="grid grid-cols-2 gap-2 text-sm">
                          <div>
                            <span className="text-muted-foreground">Email:</span> {request.user.email}
                          </div>
                          <div>
                            <span className="text-muted-foreground">Phone:</span> {request.user.phone_number}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="mb-4 p-3 bg-accent/20 rounded-lg">
                        ...
                        <div className="text-sm">
                          {request.shipping_address && (
                            <>
                              <p className="font-medium">{request.shipping_address.full_name}</p>
                              <p>{request.shipping_address.phone_number}</p>
                              <p className="mt-2">{request.shipping_address.address}</p>
                              {(request.shipping_address.city || request.shipping_address.state) && (
                                <p>
                                  {[request.shipping_address.city, request.shipping_address.state].filter(Boolean).join(', ')}
                                </p>
                              )}
                              <p>{request.shipping_address.postal_code}</p>
                              <p>{request.shipping_address.country}</p>
                              {request.shipping_address.address_notes && (
                                <p className="mt-2 text-muted-foreground">Notes: {request.shipping_address.address_notes}</p>
                              )}
                            </>
                          )}
                        </div>
                      ...
                      {request.status === 'rejected' && request.rejection_details && (
                        <div className="mb-4 p-3 bg-destructive/10 border border-destructive/20 rounded-lg">
                          <h4 className="font-medium mb-2 text-destructive">Rejection Details</h4>
                          <p className="text-sm mb-2">
                            <span className="font-medium">Reason:</span>{' '}
                            {request.rejection_details.rejection_reason}
                          </p>
                          <p className="text-xs text-muted-foreground mb-3">
                            Rejected on {new Date(request.rejection_details.rejected_at).toLocaleString()}
                          </p>

                          {Array.isArray(request.rejection_details.product_issues) && request.rejection_details.product_issues.length > 0 && (
                            <div className="space-y-4">
                              {/* ⚠️ Flagged Products */}
                              {request.rejection_details.product_issues.some((i: any) => i.has_issue) && (
                                <div>
                                  <AlertTriangle className="h-4 w-4 text-destructive mt-0.5 flex-shrink-0" />
                                  <p className="text-sm font-semibold text-destructive mb-1">Flagged Products</p>
                                  {request.rejection_details.product_issues
                                    .filter((issue: any) => issue.has_issue)
                                    .map((issue: any, index: number) => (
                                      <div key={index} className="pl-3 border-l-2 border-destructive/30">
                                        <p className="text-sm font-medium text-destructive">
                                          {issue.item_name || 'Item ' + (index + 1)}
                                        </p>
                                        <p className="text-xs text-muted-foreground">
                                          Qty: {issue.quantity} | Weight: {issue.weight}g
                                        </p>
                                        ...
                                        {issue.product_url && (
                                          ...
                                          >
                                            {issue.product_url.length > 60
                                              ? issue.product_url.substring(0, 60) + '...'
                                              : issue.product_url}
                                          </a>
                                        )}
                                      </div>
                              ...
                              {request.rejection_details.product_issues.some((i: any) => !i.has_issue) && (
                                <div>
                                  <p className="text-sm font-semibold text-green-700 mb-1">Products</p>
                                  {request.rejection_details.product_issues
                                    .filter((issue: any) => !issue.has_issue)
                                    .map((issue: any, index: number) => (
                      ...
                      {!(request.status === 'rejected' && request.rejection_details) && (
                        <div className="mb-4 p-3 bg-accent/20 rounded-lg">
                          <h4 className="font-medium mb-2">Items ({request.items?.length || 0})</h4>
                                  ...
                                  {item.is_box ? (
                                    item.local_tracking_number && (
                                      <div className="ml-5 text-xs text-muted-foreground">
                                        Local Tracking: {item.local_tracking_number}
                                      </div>
                                    )
                                  ) : (
                                    item.product_url && (
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                          <div>
                            ...
                            <p className="font-medium">¥{request.estimated_cost || '-'}</p>
                          </div>
                          <div>
                            <p className="text-sm text-muted-foreground">Actual Cost</p>
                            <p className="font-medium">¥{request.actual_cost || '-'}</p>
                          </div>
                        </div>
                        <div className="flex gap-2">
                          {request.status === 'pending' && (
                            <>
                              ...
                              <Button
                                ...
                                onClick={() => handleCancelShipment(request.id)}
                              >
                          ...
                          {request.status === 'quoted' && (
                            <>
                              <Button
                                ...
                                onClick={() => handleOpenConfirmPaymentDialog(request)}
                              >
                          ...
                          {request.status === 'paid' && (
                            <Button
                              ...
                              onClick={() => handleOpenMarkAsSentDialog(request)}
                            >
      ...
      <AlertDialog open={rejectDialogOpen} onOpenChange={setRejectDialogOpen}>
        <AlertDialogContent className="max-w-2xl">
          ...
          <div className="space-y-4">
            ...
            <div>
              <Label htmlFor="rejection-reason">Overall Rejection Reason</Label>
              <Textarea
                id="rejection-reason"
                ...
                <ScrollArea className="h-[300px] w-full rounded-md border p-4">
                  <div className="space-y-4">
                    {rejectingRequest.items.map((item, index) => {
                      const issue = productIssues.find(pi => pi.productId === item.id);
                      return (
                        <div key={item.id || index} className="space-y-2 p-3 border rounded-lg">
                          <div className="flex items-start space-x-2">
                            <Checkbox
                              id={`issue-${item.id}`}
                              checked={issue?.hasIssue || false}
                              onCheckedChange={(checked) =>
                                updateProductIssue(item.id, 'hasIssue', checked as boolean)
                              }
                            />
                          ...
                          {issue?.hasIssue && (
                            <Textarea
                              ...
                              onChange={(e) =>
                                updateProductIssue(item.id, 'issueDescription', e.target.value)
                              }
      ...
      <Dialog open={editTrackingDialogOpen} onOpenChange={setEditTrackingDialogOpen}>
        <DialogContent>
          <DialogHeader>
      ...
      <Dialog open={editBoxTrackingDialogOpen} onOpenChange={setEditBoxTrackingDialogOpen}>
        <DialogContent>
          <DialogHeader>
      ...
      <AlertDialog open={cancelDialogOpen} onOpenChange={setCancelDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to cancel this shipping request? The customer will be notified and the items will be freed back to their storage.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => {
      ...
      <AlertDialog open={confirmPaymentDialogOpen} onOpenChange={setConfirmPaymentDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to confirm payment for Shipment #{paymentRequestForConfirmation?.shipment_personal_id}? 
              This will mark the shipping quote as paid and notify the customer.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <AlertDialog open={markAsSentDialogOpen} onOpenChange={setMarkAsSentDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to mark Shipment #{requestForMarkAsSent?.shipment_personal_id} as sent? 
              You will be able to add tracking information in the next step.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
