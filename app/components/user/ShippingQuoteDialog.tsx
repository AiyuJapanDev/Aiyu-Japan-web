...
const ShippingQuoteDialog: React.FC<ShippingQuoteDialogProps> = ({
  ...
  const shippingCostResult = selectedCountry && !settingsLoading
    ? calculateShippingCostByCountry(
        selectedCountry, 
        shippingMethod, 
        numericWeight,
  ...
  const shippingCost = shippingCostResult 
    ? (typeof shippingCostResult === 'number' ? shippingCostResult : shippingCostResult.total)
    : null;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast({
        ...
        description:
          t('shippingQuoteRequestedSuccess')
      });
            ...
            <div className="space-y-2 max-h-32 overflow-y-auto text-sm font-body text-gray-700">
              {selectedItems.map(item => (
                <div key={item.id} className="flex justify-between">
                  <span>{item.product_request.item_name} ×{item.product_request.quantity}</span>
