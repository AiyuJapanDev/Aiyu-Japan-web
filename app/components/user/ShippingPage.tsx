...
export const ShippingPage = () => {
          ...
          const enrichedItems = await Promise.all(
            (quote.items as any[]).map(async (item: any, index: number) => {
              ...
              if (item.order_item_id) {
                ...
                const { data: productRequestData, error: prError } = await supabase
                  .from('product_requests')
                  .select('product_url, item_name, quantity, weight, has_shipping_issue, shipping_issue_description, is_box, local_tracking_number')
  ...
  const getShippingQuoteSteps = (status: string) => {
    ...
    if (status === 'rejected') {
      return [
        { label: t('requestSubmittedStatus'), status: 'completed' as StepStatus, icon: <Package className="h-4 w-4" /> },
        { label: t('statusRejectedBadge'), status: 'rejected' as StepStatus, icon: <AlertCircle className="h-4 w-4" /> }
      ];
    }
    ...
    if (status === 'cancelled') {
      return [
        { label: t('requestSubmittedStatus'), status: 'completed' as StepStatus, icon: <Package className="h-4 w-4" /> },
        { label: t('statusCancelled'), status: 'rejected' as StepStatus, icon: <X className="h-4 w-4" /> }
      ];
    }

    const steps = [
      { label: t('requestSubmittedStatus'), status: 'upcoming' as StepStatus, icon: <Package className="h-4 w-4" /> },
      { label: t('awaitingPaymentStatus'), status: 'upcoming' as StepStatus, icon: <CreditCard className="h-4 w-4" /> },
      { label: t('paymentReceivedStatus'), status: 'upcoming' as StepStatus, icon: <DollarSign className="h-4 w-4" /> },
      { label: t('itemsShipped'), status: 'upcoming' as StepStatus, icon: <Truck className="h-4 w-4" /> }
    ];
  ...
  const getShippingSteps = () => {
    ...
    return [
      { label: 'Processing', status: 'completed' as StepStatus, icon: <Clock className="h-4 w-4" /> },
      { label: 'Shipped', status: 'current' as StepStatus, icon: <Truck className="h-4 w-4" /> },
      { label: 'In Transit', status: 'upcoming' as StepStatus, icon: <Package className="h-4 w-4" /> },
      { label: 'Delivered', status: 'upcoming' as StepStatus, icon: <CheckCircle className="h-4 w-4" /> }
    ];
  };

  if (loading) {
    return <div className="flex justify-center p-8">{t('loadingShipments')}</div>;
  }
  ...
  if (statusFilter === "awaiting_quote") {
    ...
    filteredShipments = allShipments.filter(s =>
      s.type === 'processed_quote' && s.status === 'quoted'
    );
  } else if (statusFilter === "paid") {
    filteredShipments = allShipments.filter(s =>
      s.type === 'processed_quote' && s.status === 'paid'
    );
  } else if (statusFilter === "shipped") {
    filteredShipments = allShipments.filter(s =>
      (s.type === 'processed_quote' && (s.status === 'shipped' || s.status === 'sent')) ||
      s.type === 'shipped_order'
    );
  } else if (statusFilter === "rejected") {
    filteredShipments = allShipments.filter(s =>
      s.type === 'processed_quote' && s.status === 'rejected'
    );
  } else if (statusFilter === "cancelled") {
    filteredShipments = allShipments.filter(s =>
      s.type === 'processed_quote' && s.status === 'cancelled'
    );
  }
  ...
  if (hideRejected) {
    filteredShipments = filteredShipments.filter(s =>
      !(s.type === 'processed_quote' && (s.status === 'rejected' || s.status === 'cancelled'))
    );
  }
  ...
  const paginatedPendingQuotes = paginatedShipments.filter(s => s.type === 'pending_quote') as (ShippingQuote & { type: 'pending_quote' })[];
  const paginatedProcessedQuotes = paginatedShipments.filter(s => s.type === 'processed_quote') as (ShippingQuote & { type: 'processed_quote' })[];
  const paginatedShippedOrders = paginatedShipments.filter(s => s.type === 'shipped_order') as (OrderWithDetails & { type: 'shipped_order' })[];
  ...
  return (
    <div className="space-y-6">
        ...
        <CollapsibleContent className="overflow-hidden data-[state=open]:animate-collapsible-down data-[state=closed]:animate-collapsible-up">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <Select value={statusFilter} onValueChange={setStatusFilter}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="awaiting_quote">{t('statusAwaitingQuote')}</SelectItem>
                    <SelectItem value="quoted">{t('statusQuoted')}</SelectItem>
                    <SelectItem value="paid">{t('statusPaymentReceived')}</SelectItem>
                    <SelectItem value="shipped">{t('statusShippedBadge')}</SelectItem>
                    <SelectItem value="rejected">{t('statusRejectedBadge')}</SelectItem>
              ...
              {/* Show Per Page */}
              <div className="flex items-center gap-2">
                <span className="text-sm font-medium">{t('showPerPage')}</span>
                <Select
          ...
          {filteredShipments.length > 0 && (
            <div className="mt-4 text-sm text-muted-foreground">
              {t('showingShipments')} {startIndex + 1}-{Math.min(endIndex, filteredShipments.length)} {t('ofShipments')}{" "}
              {filteredShipments.length} {t('shipmentsText')}
            </div>
          )}
        </CardContent>
      </Card>
      ...
      {filteredShipments.length === 0 && (shippingQuotes.length > 0 || orders.length > 0) && (
        <Card>
          <CardContent className="p-8 text-center">
            <Truck className="h-12 w-12 text-muted-foreground mx-auto mb-4" />
            <p className="text-muted-foreground">{t('noShipmentsWithFilter')}</p>
            <p className="text-sm text-muted-foreground mt-2">{t('noShipmentsWithFilter')}</p>
          </CardContent>
        </Card>
      )}
            ...
            return (
              <Collapsible key={quote.id} open={isOpen}>
                <Card id={`shipment-${quote.id}`} className="border-primary/20">
                  <CardHeader className="cursor-pointer" onClick={() => toggleItem(`quote-${quote.id}`)}>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                      {t('requestedOn')} {new Date(quote.created_at).toLocaleDateString()} •
                      {totalItems} {totalItems === 1 ? 'item' : 'items'} •
                      {quote.total_weight}g •
                      {quote.destination} •
                    ...
                    <CardContent className="space-y-4">
                      ...
                      {quote.status === 'rejected' && quote.rejection_reason && (
                        <Alert variant="destructive">
                          ...
                          <AlertDescription>{quote.rejection_reason}</AlertDescription>
                        </Alert>
                      )}
                      ...
                      {quote.quote_url && quote.status === 'quoted' && (
                        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                          <div className="flex items-center justify-between flex-wrap gap-2">
                            <div>
                              <p className="text-sm font-medium text-blue-900">{t('paymentLinkAvailable')}</p>
                              <p className="text-xs text-blue-700 mt-1">{t('clickToPay')}</p>
                            </div>
                            <div className="flex items-center gap-2">
                              <Button
                                size="sm"
                      ...
                      {quote.quote_url && (quote.status === 'paid' || quote.status === 'shipped' || quote.status === 'sent') && (
                        <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                          <div className="flex items-center justify-between">
                            <div>
                              <p className="text-sm font-medium text-green-900">{t('paymentConfirmed')}</p>
                              <p className="text-xs text-green-700 mt-1">{t('shippingPaymentReceived')}</p>
                            </div>
                            <Button
                              size="sm"
                              variant="outline"
                        ...
                        if (boxItems.length > 0) {
                          ...
                          return (
                            <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
                              <div className="mb-3">
                                <p className="text-sm font-medium text-purple-900">Track Your Boxes</p>
                                <p className="text-xs text-purple-700 mt-1">Each box can be tracked individually</p>
                              </div>
                              <div className="space-y-3">
                                {boxItems.map((boxItem, index) => {
                                  ...
                                  const trackingEntry = quote.tracking_urls?.find(
                                    entry => entry.item_id === boxItem.order_item_id
                                  );
                                  
                                  return (
                                    <div key={boxItem.order_item_id || index} 
                                      ...
                                      <div className="flex items-center gap-2">
                                        ...
                                        <div>
                                          <p className="text-sm font-medium text-purple-900">
                                            {boxItem.item_name || `Box ${index + 1}`}
                                          </p>
                                          {boxItem.local_tracking_number && (
                                            <p className="text-xs text-purple-600">Tracking: {boxItem.local_tracking_number}</p>
                                          )}
                                        </div>
                                      </div>
                          ...
                          return (
                            <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
                              <div className="flex items-center justify-between">
                                <div>
                                  <p className="text-sm font-medium text-purple-900">{t('shipmentTracking')}</p>
                                  <p className="text-xs text-purple-700 mt-1">{t('shipmentTrackingInfo')}</p>
                                </div>
                                <Button
                                  size="sm"
                                  variant="outline"
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            {quote.estimated_cost && (
                              <div>
                                ...
                                <p className="text-lg font-semibold">¥{quote.estimated_cost.toLocaleString()}</p>
                              </div>
                            )}
                            {quote.actual_cost && (
                              <div>
                                <p className="text-sm text-muted-foreground">{t('finalQuote')}</p>
                                <p className="text-lg font-semibold text-primary">¥{quote.actual_cost.toLocaleString()}</p>
                              </div>
                            )}
                          </div>
                      ...
                      <div className="space-y-2">
                        <h4 className="font-medium text-sm">{t('itemsInShipment')}</h4>
                        {quote.status === 'rejected' && quote.rejection_details?.product_issues ? (
                          ...
                          quote.rejection_details.product_issues.map((issue: any, index: number) => {
                              ...
                              >
                                <div className="flex-1">
                                  <div className="flex items-start gap-2">
                                    ...
                                    <div className="flex-1">
                                      <p className={`text-sm font-medium ${hasIssue ? 'text-destructive' : ''}`}>
                                        {issue.item_name || 'Unnamed Product'}
                                      </p>

                                      <ItemIdentifier item={issue} />

                                      {hasIssue && issue.issue_description && (
                                        <p className="text-xs text-destructive/80 mt-1">
                                         {t('issuePrefix')} {issue.issue_description}
                                        </p>
                                      )}
                                    </div>
                                  </div>
                                ...
                                <div className="text-right">
                                  <span className="text-sm text-muted-foreground">Qty: {issue.quantity || 1}</span>
                                  {issue.weight && (
                                    <span className="text-xs text-muted-foreground block">{issue.weight}g</span>
                                  )}
                                </div>
                              </div>
                            );
                          ...
                          quote.items.map((item: ShippingQuoteItem, index: number) => {
                            ...
                            return (
                              ...
                              >
                                <div className="flex-1">
                                  <div className="flex items-start gap-2">
                                    ...
                                    <div className="flex-1">
                                      ...
                                      {hasIssue && item.shipping_issue_description && (
                                        <p className="text-xs text-destructive/80 mt-1">
                                          {t('issuePrefix')} {item.shipping_issue_description}
                                        </p>
                                      )}
                                    </div>
                                  </div>
                                ...
                                <div className="text-right">
                                  <span className="text-sm text-muted-foreground">Qty: {item.quantity || 1}</span>
                                  {item.weight && (
                                    <span className="text-xs text-muted-foreground block">{item.weight}g</span>
                                  )}
                                </div>
                              </div>
                            );
                      ...
                      {quote.shipping_address && (
                        <div className="border-t pt-4">
                          <h4 className="font-medium text-sm mb-2">{t('shippingAddressLabel')}</h4>
                          <div className="text-sm text-gray-700 bg-gray-50 border border-gray-200 rounded-lg p-3 space-y-1">
                            <p><span className="font-medium">{t('addressName')}</span> {quote.shipping_address.full_name}</p>
                            <p><span className="font-medium">{t('addressPhone')}</span> {quote.shipping_address.phone_number}</p>
                            <p><span className="font-medium">{t('address')}</span> {quote.shipping_address.address}</p>
                            {quote.shipping_address.address_notes && (
                              <p><span className="font-medium">{t('addressNotes')}</span> {quote.shipping_address.address_notes}</p>
                            )}
                            <p>
                              <span className="font-medium">{t('cityState')}</span>{' '}
                              {quote.shipping_address.city || '-'}, {quote.shipping_address.state || '-'}
                            </p>
                            <p>
                              <span className="font-medium">{t('postalCode')}</span> {quote.shipping_address.postal_code}
                            </p>
                            <p>
                              <span className="font-medium">{t('country')}</span> {quote.shipping_address.country}
                            </p>
                          </div>
                        </div>
                      )}
            ...
            return (
              <Collapsible key={quote.id} open={isOpen}>
                <Card className="border-primary/20">
                  <CardHeader onClick={() => toggleItem(`quote-${quote.id}`)}
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <Badge
                          className={
                            quote.status === 'quoted' ? 'bg-blue-100 text-blue-800 border-blue-200' :
                              quote.status === 'paid' ? 'bg-green-100 text-green-800 border-green-200' :
                                quote.status === 'rejected' ? 'bg-red-100 text-red-800 border-red-200' :
                                  quote.status === 'cancelled' ? 'bg-gray-100 text-gray-800 border-gray-200' :
                                    'bg-purple-100 text-purple-800 border-purple-200'
                          }
                        >
                          {quote.status === 'quoted' && t('statusAwaitingPaymentBadge')}
                  ...
                  <CollapsibleContent className="overflow-hidden data-[state=open]:animate-collapsible-down data-[state=closed]:animate-collapsible-up">
                    <CardContent className="space-y-4">
                      ...
                      {(quote.status === 'shipped' || quote.status === 'sent') && (
                        <>
                          {quote.tracking_urls && quote.tracking_urls.length > 0 ? (
                            <div className="bg-purple-50 border border-purple-200 rounded-lg p-4 space-y-3">
                              <p className="text-sm font-medium text-purple-900">{t('shipmentTracking')}</p>
                              {quote.tracking_urls.map((entry, index) => (
                                <div key={entry.item_id} className="flex items-center justify-between gap-2 py-2 border-t first:border-t-0">
                                  <div className="flex items-start gap-2 flex-1">
                                    <Package className="h-4 w-4 text-purple-700 mt-0.5 flex-shrink-0" />
                                    <div>
                                      <p className="text-sm text-purple-900">
                                        {quote.items.find(item => item.order_item_id === entry.item_id)?.item_name || 'Package'}
                                      </p>
                                    </div>
                                  </div>
                                  <Button
                                    ...
                                    onClick={() => window.open(entry.tracking_url, '_blank')}
                                  >
                                    <Truck className="h-4 w-4 mr-1" />
                                    {t("trackPackage")}
                                  </Button>
        ...
        <div className="space-y-4">
          ...
          {paginatedShippedOrders.map(order => {
            ...
            const totalItems = order.order_items?.reduce((sum, item) =>
              sum + (item.product_request.quantity || 1), 0
            ) || 0;
              ...
              <Collapsible key={order.id} open={isOpen}>
                <Card>
                  <CardHeader>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                      Shipped on {order.shipped_at ? new Date(order.shipped_at).toLocaleDateString() : 'N/A'} • {totalItems} {totalItems === 1 ? 'item' : 'items'}
                    </p>
                  </CardHeader>
                  ...
                  <CollapsibleContent className="overflow-hidden data-[state=open]:animate-collapsible-down data-[state=closed]:animate-collapsible-up">
                    <CardContent className="space-y-4">
                      ...
                      <div className="space-y-2">
                        <h4 className="font-medium text-sm">Items in this shipment:</h4>
                        {order.order_items?.map(item => (
                          <div key={item.product_request.id} className="flex items-center justify-between p-2 bg-secondary/30 rounded">
                            <p className="text-sm">{item.product_request.item_name || 'Unnamed Product'}</p>
                            <span className="text-xs text-muted-foreground">Qty: {item.product_request.quantity || 1}</span>
                          </div>
                        ))}
                      </div>
                    </CardContent>
      ...
      <AlertDialog open={!!cancelShipmentId} onOpenChange={(open) => !open && setCancelShipmentId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
