-- Create function to automatically group product requests into orders
...
-- Create trigger to automatically create orders when product requests are submitted
DROP TRIGGER IF EXISTS auto_create_order_on_product_request ON public.product_requests;
CREATE TRIGGER auto_create_order_on_product_request
AFTER INSERT ON public.product_requests
FOR EACH ROW
EXECUTE FUNCTION public.create_order_for_product_requests();